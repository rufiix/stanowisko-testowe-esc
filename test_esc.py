# --- OSTATECZNA WERSJA 14.0 - POPRAWIONA LOGIKA PĘTLI SYGNAŁOWEJ ---
import time
import random
import threading
import pigpio
...
class ESCTestStand:
    def __init__(self, gpio_pin):
        ...
        self._signal_loop_thread = threading.Thread(target=self._run_signal_loop, daemon=True)
        self._signal_loop_thread.start()

    # <<< POPRAWKA: Zmieniono warunek, aby pętla działała też podczas pauzy >>>
    def _run_signal_loop(self):
        """Ta pętla działa cały czas w tle, wysyłając sygnał IDLE, gdy test nie jest aktywny."""
        while True:
            # Jeśli system jest uzbrojony ORAZ (test nie jest uruchomiony LUB jest spauzowany)
            if self._is_armed and (not self._is_running or self._is_paused):
                # ...ciągle wysyłaj sygnał IDLE
                self._current_pwm = self.idle_pwm
                self.pi.set_servo_pulsewidth(self.gpio_pin, int(self.idle_pwm))
            time.sleep(0.1)
    ...
    def _run_test_loop(self):
        ...
        while self._is_running and time.time() < self._end_time:
            if self._is_paused:
                last_update_time = time.time() # Reset czasu, żeby uniknąć skoku
                time.sleep(0.1)
                continue
...
@ui.page('/')
def main_page():
    # --- UI pozostaje bez zmian, jest poprawne ---
    # (Poniżej pełny kod UI dla 100% pewności)
    if pigpio_error:
